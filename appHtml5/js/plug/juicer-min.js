...
/**
 * @desc 请求url,获取数据，并用数据初始化页面模板
 * @author fengshuonan
 * @param {Object} parmsObj{
 * 		tplId : 模板的id
 * 		contentId: 套用模板的id
 * 		url : 请求获取数据的网址
 * 		data : 请求网页的数据
 * 		cbackfun : 请求网页成功后的回调
 * 		afterLoad: 加载完模板数据之后执行的函数
 * }
 */
function juicerTpl(parmsObj){
	
	var backfun = function(data){
		
		var tpl = document.getElementById(parmsObj.tplId).innerHTML;
		var html = '';
		
		if( data.responseBody instanceof Array )
		{
			var temp = {list:data.responseBody};
			html = juicer(tpl, temp);
		}else{
			html = juicer(tpl, data.responseBody);
		}
		
		$("#"+parmsObj.contentId).html(html);
		
		if(parmsObj.afterLoad){
			var fun = parmsObj.afterLoad;
			fun(data);
		}
	};
	
	if(parmsObj.cbackfun)
	{
		backfun = parmsObj.cbackfun;
	}
